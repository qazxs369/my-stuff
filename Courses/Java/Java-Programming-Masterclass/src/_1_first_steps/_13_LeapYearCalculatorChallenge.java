...
public class _13_LeapYearCalculatorChallenge {
...
NOTE: Do not add a  main method to solution code.
    * */
    ...
    public static boolean isEvenlyDivisibleBy (int num, int div) {
        return num % div == 0;
    }
    ...
    public static boolean isLeapYear (int year){
        if  (1 <= year && year <= 9999) {
            if (isEvenlyDivisibleBy(year,4)){
                if (isEvenlyDivisibleBy(year,100)){
                    if (isEvenlyDivisibleBy(year,400)){
                        return true;
                    } else {
                        return false;
                    }
